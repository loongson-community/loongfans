  ...
  return new Promise((resolve, reject) => {
    ...
    const check = () => {
      ...
      if (retries >= maxRetries) {
        reject(
          new Error(
            `File ${filePath} not found or empty after ${maxRetries} retries`,
          ),
        )
        return
      }
...
async function validateData() {
  ...
  try {
    await Promise.all([waitForFile(osDataPath), waitForFile(chipsDataPath)])
  } catch (error) {
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      ...
      server.httpServer?.once("listening", () => {
        console.log(
          "[json-validator] Waiting for data generation before validation...",
        )
    ...
    buildStart() {
      if (process.env.NODE_ENV === "production") {
        console.log(
          "[json-validator] Starting validation for production build...",
        )
