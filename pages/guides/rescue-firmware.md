...
> 注：若从华硕下载固件，请验证其 zip 文件，因为华硕提供的是 zip 文件的 SHA-256 哈希，不影响后续刷写操作，验证后解压即可。

若使用 Windows，推荐使用 OpenHashTab 进行验证：
![](/images/guides/rescue-firmware/verify-with-openhashtab.png)

若使用 Linux 发行版，可使用 Dolphin 自带的校验功能：
![](/images/guides/rescue-firmware/verify-with-dolphin.png)
