...
layout: page
pageTitle: 龙架构上游动向
pageSubTitle: 2026.06.06-2026.06.12
---

> 本文包括了龙架构近一周（截至 2026 年 6 月 12 日）在 Linux 内核、工具链、Box64、EDK II 及发行版等方面的技术进展。
...
Zeng Chi 根据 Bibo Mao [提出](https://lore.kernel.org/loongarch/75a7525a-28d9-fdcb-a696-e56942530e7c@loongson.cn/)的修改意见[提交了](https://lore.kernel.org/loongarch/20260605083108.2534481-1-zeng_chi911@163.com/) 第 3 版补丁，仅做了部分代码调整及主题前缀修改，无代码更改。
...
2026 年 6 月 4 日，Xuewen Wang [修复了](https://lore.kernel.org/loongarch/20260604090151.3254805-1-wangxuewen@kylinos.cn/)龙架构  `set_direct_map_valid_noflush()` 函数中的参数传递错误的问题.已由审阅者 Huacai Chen [合并](https://lore.kernel.org/loongarch/CAAhV-H4++mQ4LHdE7_nG4xVQ5BuJm65HP9JVLgsk9x7_RGqh4A@mail.gmail.com/)。

2026 年 6 月 4 日，Qiang Ma [修复了](https://lore.kernel.org/loongarch/20260604123433.3182173-1-maqianga@uniontech.com/)龙架构 KVM 中 `kvm_emu_xchg_csr()` 函数对 CSRXCHG 指令的模拟错误。已由审阅者 Huacai Chen [合并](https://lore.kernel.org/loongarch/CAAhV-H7pKtVioksaR7ZnM-9R3=TW_-AWhzFkC80GD+RW4uWWDQ@mail.gmail.com/)。

2026 年 6 月 3 日，Bibo Mao [修复了](https://lore.kernel.org/loongarch/20260603023430.1748197-1-maobibo@loongson.cn/)龙架构 KVM 的 FPU 寄存器用户态访问接口在引入 LSX/LASX 支持后产生的 FPU 数据宽度不足的问题。已由审阅者 Huacai Chen [合并](https://lore.kernel.org/loongarch/CAAhV-H53PNRdH02c9O8ngdoitWts6=t-42Xa1EvWgLS+05qznA@mail.gmail.com/)。

2026 年 5 月 31 日，Yanfei Xu [提交了](https://lore.kernel.org/loongarch/20260531135326.2238555-1-yanfei.xu@bytedance.com/)为龙架构 KVM 添加 `irqchip` 索引验证的第 2 版补丁。已由审阅者 Huacai Chen [合并](https://lore.kernel.org/loongarch/CAAhV-H6Gk4dDek5HnKx5f_oSZ00ZWnk5Nf5ygiGKE22z8vDCqw@mail.gmail.com/)。

2026 年 5 月 29 日，Eric Biggers [提交了](https://lore.kernel.org/loongarch/20260529233208.8703-1-ebiggers@kernel.org/)删除龙芯平台随机数生成器驱动的补丁。已由审阅者 Herbert Xu [合并](https://lore.kernel.org/loongarch/aip2ULHUVT8YiMuh@gondor.apana.org.au/)。
...
[heiher](https://github.com/heiher) [修复了](https://github.com/rust-lang/miri/pull/5102) [Miri](https://github.com/rust-lang/miri)（Rust的中级中间表示解释器）在龙架构上模拟 CRC 内部函数时的语义不匹配的问题。龙架构</span>硬件 CRC 指令仅消耗输入操作数的低 8/16/32 位而忽略高位，但 Miri 将完整输入直接传递给 `compute_crc32`（该函数期望指定宽度以上的所有位为零），导致符号扩展的输入触发 panic；修复通过在调用 `compute_crc32` 之前对输入值进行掩码（如 `input & 0xFF`）以屏蔽高位，使 Miri 行为与硬件一致。
