...
| ----------- | ---- | ------------- | ---- | ----- |
| PixArt PS/2 Devices | Feature | `MOUSE_PS2_PIXART` (bool: y/n) | [1](https://github.com/AOSC-Tracking/linux/commit/f2bb6ff4f797) | For laptops such as the Tongfang Chaorui L860-T2 and EA EXCELSIOR L71 based on 3A5000 and 3A6000, this patch fixes an issue where the touchpads were misidentified as PS/2 mice, breaking support for gestures and palm detection. |
| HWMon support (thermal and other forms of hardware monitoring), providing CPU thermal control for the Loongson 3 family | Feature | `CPU_HWMON` (bool: y/n) | [1](https://github.com/AOSC-Tracking/linux/commit/ab7b90e7808a) | Does not include support for thermal monitoring of the 7A bridge chip; on 3C6000 and newer platforms the version 202511 or newer firmware already represents this sensor as an ACPI Thermal Zone, with that this patch is unneeded |
| 2K3000/3B6000M SoC CAN-FD Bus | Feature | `CAN_LSCANFD` (bool: y/n), `CAN_LSCANFD_PLATFORM` (tristate: y/m/n) | [1](https://github.com/AOSC-Tracking/linux/commit/0dc2107b57a2) | Must be used together with Linux 7.1 or newer kernel with multi-channel DMA support |
| BPI1000/1001 (“Old World”) firmware support | Feature | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/12790a1e5d40), [2](https://github.com/AOSC-Tracking/linux/commit/42046c2ed32d), [3](https://github.com/AOSC-Tracking/linux/commit/12e17687f2f7), [4](https://github.com/AOSC-Tracking/linux/commit/8dd6b7a65a68), [5](https://github.com/AOSC-Tracking/linux/commit/08d7416a7b74), [6](https://github.com/AOSC-Tracking/linux/commit/d83315017a3a), [7](https://github.com/AOSC-Tracking/linux/commit/bb5ee2c6ffd9), [8](https://github.com/AOSC-Tracking/linux/commit/2c89d5c8e77e) | Essential for booting ABI2.0 systems on Lenovo Kaitian M540z, Gooxi 3C5000L quad-socket servers, and certain platforms shipping Kunlun firmware from 2020 to 2022. |
| Loongson I2C controller clocks property parsing and bus speed adjustment support | Feature | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/42d158cdba34) | The lack of this feature causes the touchpad of some 3B6000M laptops to lag. |
| SMCv2 firmware protocol update | Feature | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/e838f8194a49) | The lack of this update may cause boot failure on 202605 firmware release if DVFS is enabled; also note that the 202605 release is experimental |
| Workaround for inoperable GPIO due to device descriptions that only complies with an old release of *Loongson CPU Unified System Architecture Specification*, where GPIO devices were described within `gsi_idx_map` | Workaround | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/911ae66f00b3) | Should not affect 2K3000/3B6000M and later products |
| Workaround for intermittent driver crashes, resets, and lockups with AMD GCN 1.0–4.0 GPUs on LoongArch platforms | Workaround | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/0d9e47e4c3ad) | The mechanism behind this workaround is unclear (empirical patch); the workaround fails to evade those issues completely, it only decreases their probability; commercial 6.6 kernels like deepin include a more aggressive (yet similarly unexplained) patchset. See [this deepin pull request](https://github.com/deepin-community/kernel/pull/1215). |
| Workaround for data errors that on 7A platforms when using AMD “Radeon” graphics drivers (for TeraScale 2 and earlier graphics cards) | Workaround | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/608cc0997567), [2](https://github.com/AOSC-Tracking/linux/commit/ad49de48bb10), [3](https://github.com/AOSC-Tracking/linux/commit/3381349cf67f) | Mechanism unknown (the commit message is not technically sound) but effective; patch 3 limits this workaround to MIPS and LoongArch64 platforms (`MACH_LOONGSON64`) |
| Register the third PWM controller `LOON0006:03` on the 7A2000 bridge as `gsgpu_backlight` in board-level ACPI initialization code to support LoongGPU backlight control | Workaround | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/54af59a094a6) | Prerequisite patch for LoongGPU backlight support. For LoongGPU driver-related patches, please refer to [AOSC-Tracking/loonggpu-kernel-dkms @ aosc/v1.0.1-alpha-lnd25.5](https://github.com/AOSC-Tracking/loonggpu-kernel-dkms/commits/aosc/v1.0.1-alpha-lnd25.5/). |
| Enable "Remote Wake" support for USB root hubs (such as USB keyboards, mice, and other input devices) | Workaround | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/a50c62f43a4c), [2](https://github.com/AOSC-Tracking/linux/commit/5e7477d28344) | Applying this patch enables keyboard wake-up for LoongArch devices in ACPI S3 (suspend-to-RAM), but it is known to prevent some x86 laptops from entering sleep mode. Patch 2 limits this workaround to MIPS and LoongArch64 platforms (`MACH_LOONGSON64`). |
| Reset backlight enable state in S3 suspend/resume routine of the loongson-laptop driver | Workaround | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/1a659eeb6eec) | Workaround the issue in some 3B6000M laptop EC firmware causing the backlight disabled after resuming from S3 |
| Detect and disable sc.q if erratic | Workaround | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/1835688d13f3) | Needed to boot the kernel on 2K3000/3B6000M platforms with buggy firmware misconfiguring the LA364E core; the workaround is incomplete as the user-space programs may still get bad result in case they've not guarded the sc.q use via `AT_HWCAP` check; firmware update is recommended to resolve the issue |
