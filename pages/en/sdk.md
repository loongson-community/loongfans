...
| Description | Kconfig Entry | Link | Notes |
| ----------- | ------------- | ---- | ----- |
| PixArt PS/2 Devices | `MOUSE_PS2_PIXART` (bool: y/n) | [1](https://github.com/AOSC-Tracking/linux/commit/f2bb6ff4f797) | For laptops such as the Tongfang Chaorui L860-T2 and EA EXCELSIOR L71 based on 3A5000 and 3A6000, this patch fixes an issue where the touchpads were misidentified as PS/2 mice, breaking support for gestures and palm detection. |
| HWMon support (thermal and other forms of hardware monitoring), providing CPU thermal control for the Loongson 3 family | `CPU_HWMON` (bool: y/n) | [1](https://github.com/AOSC-Tracking/linux/commit/ab7b90e7808a) | Does not include support for thermal monitoring of the 7A bridge chip; on 3C6000 and newer platforms the version 202511 or newer firmware already represents this sensor as an ACPI Thermal Zone, with that this patch is unneeded |
| 2K3000/3B6000M SoC CAN-FD Bus | `CAN_LSCANFD` (bool: y/n), `CAN_LSCANFD_PLATFORM` (tristate: y/m/n) | [1](https://github.com/AOSC-Tracking/linux/commit/0dc2107b57a2) | Must be used together with Linux 7.1 or newer kernel with multi-channel DMA support |
...
| Loongson I2C controller clocks property parsing and bus speed adjustment support | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/42d158cdba34) | The lack of this feature causes the touchpad of some 3B6000M laptops to lag. |
| SMCv2 firmware protocol update | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/e838f8194a49) | The lack of this update may cause boot failure on 202605 firmware release if DVFS is enabled; also note that the 202605 release is experimental |
| Workaround for inoperable GPIO due to device descriptions that only complies with an old release of *Loongson CPU Unified System Architecture Specification*, where GPIO devices were described within `gsi_idx_map` | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/911ae66f00b3) | Should not affect 2K3000/3B6000M and later products |
| Workaround for intermittent driver crashes, resets, and lockups with AMD GCN 1.0–4.0 GPUs on LoongArch platforms | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/0d9e47e4c3ad) | The mechanism behind this workaround is unclear (empirical patch); the workaround fails to evade those issues completely, it only decreases their probability; commercial 6.6 kernels like deepin include a more aggressive (yet similarly unexplained) patchset. See [this deepin pull request](https://github.com/deepin-community/kernel/pull/1215). |
| Workaround for data errors that on 7A platforms when using AMD “Radeon” graphics drivers (for TeraScale 2 and earlier graphics cards) | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/608cc0997567), [2](https://github.com/AOSC-Tracking/linux/commit/ad49de48bb10), [3](https://github.com/AOSC-Tracking/linux/commit/3381349cf67f) | Mechanism unknown (the commit message is not technically sound) but effective; patch 3 limits this workaround to MIPS and LoongArch64 platforms (`MACH_LOONGSON64`) |
| Register the third PWM controller `LOON0006:03` on the 7A2000 bridge as `gsgpu_backlight` in board-level ACPI initialization code to support LoongGPU backlight control | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/54af59a094a6) | Prerequisite patch for LoongGPU backlight support. For LoongGPU driver-related patches, please refer to [AOSC-Tracking/loonggpu-kernel-dkms @ aosc/v1.0.1-alpha-lnd25.5](https://github.com/AOSC-Tracking/loonggpu-kernel-dkms/commits/aosc/v1.0.1-alpha-lnd25.5/). |
| Enable "Remote Wake" support for USB root hubs (such as USB keyboards, mice, and other input devices) | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/a50c62f43a4c), [2](https://github.com/AOSC-Tracking/linux/commit/5e7477d28344) | Applying this patch enables keyboard wake-up for LoongArch devices in ACPI S3 (suspend-to-RAM), but it is known to prevent some x86 laptops from entering sleep mode. Patch 2 limits this workaround to MIPS and LoongArch64 platforms (`MACH_LOONGSON64`). |
| Reset backlight enable state in S3 suspend/resume routine of the loongson-laptop driver | N/A | [1](https://github.com/AOSC-Tracking/linux/commit/1a659eeb6eec) | Workaround the issue in some 3B6000M laptop EC firmware causing the backlight disabled after resuming from S3 |
