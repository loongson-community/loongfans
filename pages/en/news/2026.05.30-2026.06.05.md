...
# 返回首页
returnLink: /news
pageTitle: LoongArch Upstream Updates
pageSubTitle: 2026.05.30 - 2026.06.05
---

> This article covers recent technical advancements in LoongArch over the past week (as of June 5, 2026), including developments in the Linux kernel, toolchains, and Box64.
...
Qiang Ma [修复了](https://lore.kernel.org/loongarch/20260604123433.3182173-1-maqianga@uniontech.com/)龙架构 KVM 中 `kvm_emu_xchg_csr()` 函数对 `CSRXCHG` 指令的模拟错误。该补丁已获得 Bibo Mao 审阅通过 (Reviewed-by) 标记。
...
Meng Qinggang [提交了](https://sourceware.org/pipermail/binutils/2026-June/149527.html)修复使用 `ld -r` 混合链接带有 relaxation信息的输入文件和不带 relaxation 信息的输入文件导致后者的对齐要求被破坏的问题的补丁，待评审。
...
[Xiaowei Xu (xuxiaowei-com-cn)](https://github.com/xuxiaowei-com-cn) [请求](https://github.com/etcd-io/etcd/issues/21877)为 [etcd](https://github.com/etcd-io/etcd)（一个分布式键值存储系统）添加龙架构支持，并说明了具体修改内容，但项目维护者 [Josh Berkus (jberkus)](https://github.com/jberkus) 说明 etcd 的所有测试都依赖 Kubernetes 的 CI 基础设施 (Prow)，此基础设施尚未支持龙架构。作者随后为 coredns（Kubernetes 的核心域名解析组件）[添加了](https://github.com/coredns/coredns/pull/8137)龙架构支持。

[Xiaowei Xu (xuxiaowei-com-cn)](https://github.com/xuxiaowei-com-cn) [请求](https://github.com/containernetworking/plugins/issues/1263)为 [CNI](https://github.com/containernetworking/plugins)（容器网络接口）核心插件添加龙架构支持，同时 [请求](https://github.com/kubernetes/sig-release/issues/3040)为 Kubernetes SIG-Release 添加龙架构支持。

[Xiaowei Xu (xuxiaowei-com-cn)](https://github.com/xuxiaowei-com-cn) 为 CNI（容器网络接口）核心插件的 CI 测试和发布流程[添加了](https://github.com/containernetworking/plugins/pull/1262)龙架构支持。
